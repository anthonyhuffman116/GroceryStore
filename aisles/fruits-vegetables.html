<!--
    P2 - Fruit & Vegetables Aisle Page
    Shi Qi Zhou - 40163947
-->
<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/css/bootstrap.min.css">
    <link rel="stylesheet" href="../css/p2.css">
    <title>Fruits & Vegetables</title>
</head>

<body>
    <header id="main-header">
        <img id="logo" class="img-fluid" src="../images/logo.png" alt="logo" />
        <img id="slogan" class="img-fluid" src="../images/slogan.png" alt="slogan" />
    </header>
    <div class="page-header">
        Online Groceries
    </div>
    <nav class="navbar navbar-expand-sm">
        <ul class="navbar-nav mr-auto">
            <li class="nav-item active">
                <a class="nav-link" href="../index.html">Home Page</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="../aisles/fruits-vegetables.html">Return to Aisle</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="../shopping-cart/index.html">Shopping Cart</a>
            </li>
        </ul>
        <ul class="navbar-nav ml-auto">
            <li class="nav-item">
                <a class="nav-link" href="../user/register.html">
                    <button class="user-button" type="button" name="user-button">
                    Register
                    </button>
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="../user/login.html">
                    <button class="user-button" type="button" name="login-button">
                    Log In
                    </button>
                </a>
            </li>
        </ul>
    </nav>

    <div class="banner">
        <marquee behavior="scroll" direction="left">
            <h6><b>FREE DELIVERY FOR ORDERS ABOVE 30 $</b></h6>
        </marquee>
    </div>
    <div class="container-fluid">
        <h1>Fruits & Vegetables</h1><br/>
        <div class="row container">
            <div class="col-md-3 col-sm-12">
                <div class="menu ">
                    <ul class="list-unstyled">
                        <a href="fruits-vegetables.html">
                            <li>Fruits & Vegetables</li>
                        </a>
                        <a href="meat-poultry.html">
                            <li>Meat & Poultry</li>
                        </a>
                        <a href="bread.html">
                            <li>Bread</li>
                        </a>
                        <a href="snacks.html">
                            <li>Snack</li>
                        </a>
                        <a href="frozen.html">
                            <li>Frozen</li>
                        </a>
                    </ul>
                </div>
            </div>

            <div class="col-md-9 col-sm-12">
                <div class="card-deck">
                    <div class="card">
                        <img class="card-img-top" src="../images/banana.jpg" alt="Bananas">
                        <div class="card-body">
                            <h5 class="card-title">Bananas</h5>
                            <p class="card-text">
                                Sold in lb
                            </p>
                            <h5 class="card-bold ">1.99 $</h5>
                            <a href="../product-descriptions/banana.html " class="stretched-link"></a>
                        </div>
                    </div>
                    <div class="card">
                        <img class="card-img-top" src="../images/banana.jpg" alt="Bananas">
                        <div class="card-body">
                            <h5 class="card-title">Bananas</h5>
                            <p class="card-text">
                                Sold in lb
                            </p>
                            <h5 class="card-bold">1.99 $</h5>
                            <a href="../product-descriptions/banana.html" class="stretched-link"></a>
                        </div>
                    </div>
                    <div class="card">
                        <img class="card-img-top" src="../images/banana.jpg" alt="Bananas">
                        <div class="card-body">
                            <h5 class="card-title">Bananas</h5>
                            <p class="card-text">
                                Sold in lb
                            </p>
                            <h5 class="card-bold">1.99 $</h5>
                            <a href="../product-descriptions/banana.html " class="stretched-link"></a>
                        </div>
                    </div>

                    <div class="card">
                        <img class="card-img-top" src="../images/lettuce.jpeg" alt="Romaine Lettuce">
                        <div class="card-body">
                            <h5 class="card-title">Romaine Lettuce</h5>
                            <p class="card-text">
                                Sold in unit
                            </p>
                            <h5 class="card-bold">5.00 $</h5>
                            <a href="../product-descriptions/lettuce.html" class="stretched-link"></a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>


    <footer>
        <a href="fruits-vegetables.html#main-header">Back to top</a>
    </footer>

</body>

</html>